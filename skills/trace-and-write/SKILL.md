...
- Begin every generated PDF with the bundled instruction-page layout. Keep the directions brief and child-friendly, and include two handwriting strips: a `Before` example with the independent-writing space empty and an `After` example with that space completed in black.
