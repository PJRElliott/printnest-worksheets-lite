...
- For animal candidates, generate against chroma green, remove the connected exterior green in the same production pass, clear residual green pixels, and verify genuine RGBA transparency before review. Present only the transparent candidate unless the user explicitly asks to inspect the green intermediate. Preserve enclosed white and light subject areas as opaque.
- Do not promote an illustration into an approved asset folder or use it as a future reference until the user approves it. After approval, record its role and SHA-256 hash in the relevant category manifest and validate every recorded exemplar hash.
