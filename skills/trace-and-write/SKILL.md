---
name: trace-and-write
description: Generate printable A4 CVC trace-and-write worksheet PDFs with ten handwriting strips per page. Use for CVC word-family tracing practice or Word Made Easy trace-and-write worksheets; do not use for unrelated phonics activity types.
---

# Trace and Write

Treat the layout, typography, palette, illustration placement, strip count, and alphabet exception below as locked defaults. Do not change them unless the user explicitly requests that specific rule to change.

Generate the requested worksheet with the bundled deterministic script:

```powershell
python scripts/generate_trace_and_write.py
```

- Use `--family at` to create one word-family worksheet, or omit it to create all families.
- Use `--vowel a` to create one workbook containing every family for a selected short vowel.
- Use `--output <path.pdf>` when the user specifies an output location.
- Begin every generated PDF with the bundled instruction-page layout. Keep the directions brief, child-friendly, and applicable to words of different lengths. Show two grey model words, those words traced in black, and an example of continuing the word with clear finger spaces. Tell learners to repeat the word only as many times as fit comfortably.
- Treat `assets/canonical_instruction_page.pdf` as the locked instruction page. Generate a first-page placeholder, then use `insert_canonical_instruction_page()` after saving so every workbook receives that exact PDF page instead of rebuilding it.
- Preserve the bundled full-page template. Use League Spartan for headings and instructions, and Edu SA Beginner only for the grey tracing words.
- Before generating or approving illustrations, read `assets/style-reference/pencil-sketch-v1/style-manifest.json`. For animals, also read `assets/style-reference/pencil-sketch-v1/animal-category.json`; treat their locked anchors, approved hashes, reference roles, composition rules, prohibited treatments, and background workflow as authoritative.
- Use the approved zoo-animal pencil-sketch treatment as the locked default illustration style. Match the reference assets in `assets/zoo-animals-pencil/`: fine, slightly uneven navy pencil outlines; visible hand-drawn contour strokes; delicate cross-hatching and short texture marks; softly layered coloured-pencil fills; and friendly, recognisable natural proportions. The result must visibly read as a traditional pencil drawing, not clean digital vector art.
- Use navy `#193A63`, muted gold `#E1AE43`, warm white `#FFF9EC`, and soft natural grey where the subject requires it. Keep colour restrained and retain visible pencil grain rather than flat or glossy fills. Do not use kawaii, chibi, smooth vector, glossy, 3D, realistic, or photorealistic imagery.
- When the user supplies a new style reference for a specific task, follow it only for that task unless they explicitly ask to replace this locked default.
- Keep cover artwork and isolated worksheet assets distinct. The composite zoo scene is a cover reference, including its grouped composition, dashed paths, stars, motion marks, plants, and decorative details. Isolated assets inherit its medium, line treatment, texture, and palette only; they contain one complete subject with none of those cover decorations.
- For animal candidates, generate against chroma green, remove the connected exterior green in the same production pass, clear residual green pixels, and verify genuine RGBA transparency before promotion. Preserve enclosed white and light subject areas as opaque.
- Do not ask the user to approve individual illustrations. Compare each candidate against the locked references and category rules, regenerate or correct it when it fails, and automatically promote it when both visual and technical checks pass. Record promoted exemplars and their SHA-256 hashes in the relevant category manifest, then validate every recorded exemplar hash.
- In the blue-and-gold animal style, all hooves and pig trotters must be solid navy `#193A63`, never gold.
- Place illustrations in front of the handwriting guides. Make only the external image background transparent; keep the complete subject, including enclosed white body areas, opaque so guide lines never show through the animal itself.
- Trim excess background whitespace when placing an illustration, then scale the visible artwork proportionally to fit the full reserved square between the upper and lower handwriting guides. Do not crop the subject.
- In paired practice strips, show the illustration on the first strip only; keep the second strip image-free and full width.
- Show each target word once in the word list, but provide two complete handwriting strips for that word. A ten-strip page therefore holds no more than five unique words.
- For alphabet practice only, provide one strip for the uppercase letter and one strip for the lowercase letter.
- Pack complete family sections into unused page space by default. A following section includes its own title, instruction, and strips; start a new page when the complete section will not fit within the ten-position capacity.
- Identify each section with a left-aligned `-ab Family Words` title, substituting the actual family ending. Do not include `Trace and Write` in the title.
- Use a 22 pt title and 10 pt instruction, both aligned to the 0.5-inch left content margin. For every heading block, use 0.254 inches (18.3 pt) above the title, 0.08 inches (5.76 pt) between title and instruction, and 0.254 inches (18.3 pt) below the instruction.
- Keep the established strip spacing within each family. When unused rows remain, remove only the unused bottom rows.
- Treat [references/cvc_word_families.json](references/cvc_word_families.json) as the canonical strict-CVC dataset. Keep it free of blends, digraphs, silent letters, proper names, and four-phoneme `x` endings.
- Preserve the established layout: 0.5-inch left, right, and bottom margins; 1-inch top margin; 0.35-inch clear gap between strips; and two tracing examples followed by writing space.
- Open or render the generated PDF when visual verification is relevant.
