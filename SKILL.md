...
- Use the bundled League Spartan Regular, Semibold, Bold, and ExtraBold fonts for all text except grey tracing words. Use Edu SA Beginner only for those tracing words.
...
- Show each family word exactly once with no repetition. Use one four-line tracing strip per unique word.
- Fix the title and all strip coordinates to the ten-row master layout. For fewer than ten words, retain the top rows in place and omit only unused rows from the bottom.
...
- Above tracing strips, identify the current family in the title, such as `Trace and Write -at Family Words`, and show the one-line instruction `Trace each word twice, then write it on your own.` inside the reserved area below the banner. This is the only exercise-page title/instruction exception.
