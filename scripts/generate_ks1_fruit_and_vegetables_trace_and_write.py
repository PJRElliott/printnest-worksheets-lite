#!/usr/bin/env python3
"""Generate KS1 fruit and vegetable trace-and-write workbooks."""
...
DATA_PATH = ROOT / "references" / "ks1_fruit_and_vegetables.json"
GENERATOR_PATH = ROOT / "skills" / "trace-and-write" / "scripts" / "generate_trace_and_write.py"
OUTPUT_DIR = ROOT / "outputs" / "fruit-and-vegetables"
...
def draw_instruction_page(pdf: canvas.Canvas, generator, example: str) -> None:
    ...
    items = [
        ("1. Grey Words", "The two food-word models are grey.", [example, example], generator.TRACE_GRAY),
        ("2. Traced Words", "The traced models are shown in black.", [example, example], black),
        ("3. Continue Writing", "Repeat the word as many times as fit comfortably.", [example] * 4, black),
    ]
...
def generate_set(generator, item: dict) -> Path:
    output = OUTPUT_DIR / f"ks1_{item['id']}_trace_and_write.pdf"
...
def main() -> None:
    ...
    for item in sets:
        if args.set in (None, item["id"]):
            generate_set(generator, item)
