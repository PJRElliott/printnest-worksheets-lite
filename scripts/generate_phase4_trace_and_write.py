...
def chunk_sections(sections: list[dict[str, object]], generator) -> list[tuple[str, list[str]]]:
    return [
...
def pack_sections(sections, capacity: int, generator):
    pages = []
    used = 0
    for section in sections:
...
def generate(output: Path, sections: list[dict[str, object]]) -> None:
    ...
    pages = pack_sections(chunk_sections(sections, generator), generator.MAX_TRACE_STRIPS, generator)
