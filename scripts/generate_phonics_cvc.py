...
def page_cover(c, meta: dict) -> None:
    ...
    colors = [PRIMARY, ACCENT, HexColor("#10B981")]
    font_name, font_size = "Helvetica-Bold", 70
    ...
    c.roundRect((PAGE_W - bw) / 2, 2.6 * inch, bw, bh, 0.15 * inch, fill=1, stroke=0)
...
def page_family_intro(c, page_num: int, family: str, words: list[str]) -> None:
    ...
    # 大きな family 表示
    c.setFillColor(SOFT_PURPLE)
    c.roundRect(MARGIN, PAGE_H - 2.3 * inch, PAGE_W - 2 * MARGIN, 0.9 * inch,
                0.15 * inch, fill=1, stroke=0)
    c.setFillColor(PRIMARY)
    c.setFont("Helvetica-Bold", 56)
    c.drawCentredString(PAGE_W / 2, PAGE_H - 2.1 * inch, family)

    # 単語をトレース行に配置（4本罫線 × N行）
    show_count = min(len(words), 6)
    for i in range(show_count):
        baseline_y = PAGE_H - 3.4 * inch - i * 1.0 * inch
...
def page_missing_letter(c, page_num: int, family: str, words: list[str],
    ...
    for i in range(show):
        baseline_y = PAGE_H - 2.2 * inch - i * 0.8 * inch
        word = words[i]
...
def page_mixed_review(c, page_num: int, rng: random.Random,
    ...
    for i, (word, fam) in enumerate(items):
        ...
        y = PAGE_H - 2.0 * inch - row * 1.2 * inch
        # 番号 + 単語
        c.setFillColor(DARK)
        c.setFont("Helvetica-Bold", 14)
...
def page_build_word(c, page_num: int, family: str, letters: list[str]) -> None:
    ...
    c.setFillColor(SOFT_YELLOW)
    c.roundRect(MARGIN, PAGE_H - 2.4 * inch, PAGE_W - 2 * MARGIN, 0.9 * inch,
                0.15 * inch, fill=1, stroke=0)
    c.setFillColor(DARK)
    c.setFont("Helvetica-Bold", 42)
    c.drawCentredString(PAGE_W / 2, PAGE_H - 2.15 * inch,
                        f"_ _   +   {family}   =   ?")
    # 候補文字
    c.setFillColor(DARK)
    c.setFont("Helvetica-Bold", 14)
    c.drawString(MARGIN + 0.3 * inch, PAGE_H - 2.9 * inch,
                 "Try these letters:  " + " · ".join(letters))
    # 書き欄 8つ
    for i in range(8):
        row, col = divmod(i, 2)
        x = MARGIN + 0.5 * inch + col * 3.4 * inch
        y = PAGE_H - 3.6 * inch - row * 1.0 * inch
        c.setFillColor(DARK)
        c.setFont("Helvetica-Bold", 14)
