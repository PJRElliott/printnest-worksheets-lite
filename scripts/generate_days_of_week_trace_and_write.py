#!/usr/bin/env python3
"""Generate the branded Days of the Week trace-and-write workbook."""
...
def draw_instruction_page(pdf: canvas.Canvas, generator) -> None:
    ...
    pdf.drawString(
        left, top - 0.34 * inch,
        "Trace each grey day twice, then continue writing with clear spaces.",
    )

    items = [
        ("1. Grey Words", "The two day-name models are grey.", ["Monday", "Monday"], generator.TRACE_GRAY),
        ("2. Traced Words", "The traced models are shown in black.", ["Monday", "Monday"], black),
        ("3. Continue Writing", "Repeat the word as many times as fit comfortably.", ["Monday"] * 3, black),
    ]
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "outputs" / "days-of-week" / "days_of_the_week_trace_and_write.pdf",
    )
